...
int main() {
    ...
    for (int i = 0; i < count; i++) {
        int collisions = hashTableInsert(hashTable, words[i], i);
    ...
    printf("Total Collisions: %d\n", totalCollisions); // Вывод общего количества коллизий
    ...
    for (int range = 1; range <= 20; range++) { // Диапазоны от 10000 до 200000 с шагом 10000
        ...
        for (int experiment = 0; experiment < 1000; experiment++) {
            ...
            start = clock();
            int result = hashTableSearch(hashTable, words[randomIndex]);
            end = clock();
